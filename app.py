...
.score-label { font-size: 14px; color: #888; margin-bottom: 2px; }
...
.huge-text { font-size: 70px !important; font-weight: 900; color: #333; margin: 10px 0; }
...
.stButton > button[key^="btn_"] {
    ...
    border: none !important;
    box-shadow: 0 4px 0 #FF477E !important;
...
for i, opt in enumerate(st.session_state.q_opts):
    ...
    if target_col.button(str(opt), key=f"btn_{opt}_{i}", use_container_width=True):
        is_correct = (st.session_state.game_mode == "数学" and str(opt) == str(st.session_state.q_ans)) or \
                     (st.session_state.game_mode == "识字" and str(opt) == str(st.session_state.w_target))
        
        if is_correct:
            # --- 答对逻辑 ---
            st.session_state.score += 1 # 分数+1
            if st.session_state.score > st.session_state.high_score:
                st.session_state.high_score = st.session_state.score # 更新最高分
            ...
            # --- 答错逻辑 ---
            st.session_state.score = 0 # 答错则分数重置（鼓励连续对）
            st.session_state.show_error = True
            st.rerun()

# 底部清零成绩按钮（可选，侧边栏）
if st.sidebar.button("重置最高纪录"):
    st.session_state.high_score = 0
    st.rerun()
